...
const {
  BRAND, logoDataUri, bufferToDataUri,
...
async function fetchPhotoBuffer(bucket, path) {
  ...
  try {
    ...
    // Surface the failure in server logs so silently-missing chaperone
    // photos in PDF / XLSX exports are diagnosable.
    console.warn('[onboarding-export] photo download failed', { path, err: err?.message || String(err) });
    return null;
  }
}
...
async function buildXlsx({ records, audit, sections, includePhotos, meta, filters }) {
  ...
  wb.created = new Date();
  const headerStyle = brandHeaderStyle('green');

  // Branded cover sheet via shared module (logo, title block, KPI grid).
  const t = summarize(records);
  const filterNotes = [];
  if (filters.status && filters.status !== 'all') filterNotes.push(`Status filter: ${filters.status}`);
  if (filters.grade)     filterNotes.push(`Grade: ${filters.grade}`);
  if (filters.homeroom)  filterNotes.push(`Homeroom: ${filters.homeroom}`);
  if (filters.studentId) filterNotes.push(`Student: ${filters.studentId}`);
  writeXlsxCoverSheet(wb, {
    title:    'Pickup Authorization Forms',
    subtitle: 'Onboarding Submissions Report',
    theme:    'green',
  ...
  if (sections.records) {
    const ws = wb.addWorksheet('Records', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
  ...
  if (sections.chaperones) {
    const ws = wb.addWorksheet('Chaperones', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
  ...
  if (sections.audit && audit?.length) {
    const ws = wb.addWorksheet('Audit', { views: [{ state: 'frozen', ySplit: 1, showGridLines: false }] });
 ...
 * attach `_facePngBuf` (Buffer | null) AND `_faceDataUri` (string | null).
 *
 * The data-URI form is what @react-pdf/renderer's <Image> actually accepts
 * — passing a raw Node.js Buffer renders nothing (silently). The Buffer
 * is kept for ExcelJS image embedding.
 */
async function attachChaperoneFaces(records) {
  let bucket = null;
  try { bucket = admin.storage().bucket(); } catch (err) {
    console.warn('[onboarding-export] storage bucket unavailable', err?.message || String(err));
    bucket = null;
  }
  if (!bucket) return;
  for (const r of records) {
    for (const c of (r.chaperones || [])) {
      ...
      if (!p) continue;
      const buf = await fetchPhotoBuffer(bucket, p);
      c._facePngBuf = buf;
      c._faceDataUri = bufferToDataUri(buf);
    }
  }
}
...
async function buildPdf({ records, audit, sections, meta, filters, includePhotos }) {
  ...
  const s = StyleSheet.create({
    ...
    letterLogo:  { width: 36, height: 36, marginRight: 10, objectFit: 'contain' },
  ...
  // Branded cover page (logo, title block, KPI grid, document info).
  const t = summarize(records);
  const filterNotes = [];
  if (filters.status && filters.status !== 'all') filterNotes.push(`Status filter: ${filters.status}`);
  if (filters.grade)     filterNotes.push(`Grade: ${filters.grade}`);
  ...
  // Optional secondary page: filter context + audit summary.
  const coverChildren = [];
  if (sections.audit && audit?.length) {
    coverChildren.push(
      e(Text, { key: 'h', style: s.coverH1 }, 'Audit Trail Summary'),
      e(Text, { key: 'm', style: s.coverMeta },
        `${audit.length} entries within reporting period (showing latest 50)`),
      e(View, { key: 'audit' },
  ...
  if (coverChildren.length) {
    pages.push(e(Page, { key: 'audit-page', size: 'A4', style: s.page },
      ...coverChildren,
  ...
  if (sections.records && records.length) {
    records.forEach((r, idx) => {
      ...
      const chapBoxes = r.chaperones.map((c, i) => {
        ...
        // @react-pdf/renderer's <Image src> needs a URL-style string,
        // not a raw Node Buffer — use the pre-computed base64 data URI.
        const photoView = (includePhotos && c._faceDataUri)
          ? e(Image, { style: s.chapPhoto, src: c._faceDataUri })
          : null;
      ...
      pages.push(e(Page, { key: `f-${r.id}-${idx}`, size: 'A4', style: s.page, wrap: true },
        e(View, { style: s.letterTop },
          e(View, { style: s.letterLeft },
            logoUri ? e(Image, { style: s.letterLogo, src: logoUri }) : null,
            e(View, null,
