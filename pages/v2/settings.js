...
export default function SettingsPage() {
  ...
  // Bulk import state (users)
  const [bulkImportTarget, setBulkImportTarget] = useState(null); // null | 'users'
  ...
  const allTabs = [
    ...
    { id: 'audit-trail', icon: 'ph-clipboard-text', label: 'Audit Trail' },
...
function AuditTrailPanel() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-light text-white tracking-wide">Audit Trail</h2>
        <p className="text-sm text-slate-500 mt-1">
          Every settings change, user-management action, device pairing and pickup override is recorded here.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="glass-panel border border-slate-700/40 rounded-xl p-4 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${TONE_CLASSES[s.tone]}`}>
              <i className={`ph ${s.icon} text-xl`} />
            </div>
            <div>
      ...
      {/* Filter bar */}
      <div className="glass-panel border border-slate-700/40 rounded-xl p-4 grid grid-cols-1 md:grid-cols-5 gap-3">
        <input
          type="text"
          placeholder="Search summary, target, JSON…"
          value={filter.q}
          onChange={(e) => setFilter(f => ({ ...f, q: e.target.value }))}
          className="bg-slate-900/60 border border-slate-700/50 rounded-lg px-3 py-2 text-sm text-white placeholder:text-slate-500 md:col-span-2"
        />
        <select
          value={filter.kind}
        ...
        <div className="flex gap-2">
          <input
            ...
            className="bg-slate-900/60 border border-slate-700/50 rounded-lg px-2 py-2 text-xs text-white flex-1"
          />
          <input
            type="date"
            value={filter.to}
        ...
        <div className="md:col-span-5 flex items-center justify-between">
          <button
            onClick={() => setFilter({ kind: '', from: '', to: '', actor: '', q: '' })}
          ...
          <button
            onClick={fetchAudit}
            disabled={loading}
            className="px-3 py-1.5 text-xs rounded-lg border border-slate-600/50 text-slate-200 hover:bg-slate-800/60 disabled:opacity-50"
          >
      ...
      {/* Entries */}
      <div className="glass-panel border border-slate-700/40 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
                ...
                return (
                  <React.Fragment key={e.id}>
                    <tr className="hover:bg-slate-800/30 cursor-pointer" onClick={() => toggle(e.id)}>
                      <td className="px-4 py-3 text-slate-300 whitespace-nowrap" title={e.at}>{timeAgo(e.at)}</td>
                      <td className="px-4 py-3">
                        <div className="text-white">{e.actor?.email || <span className="text-slate-500 italic">unknown</span>}</div>
                        {e.actor?.role && <div className="text-xs text-slate-500">{e.actor.role}</div>}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs border ${TONE_CLASSES[meta.tone]}`}>
                          <i className={`ph ${meta.icon}`} /> {meta.label}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-300">
                        {e.target?.label || e.target?.id || <span className="text-slate-500">—</span>}
                        {e.target?.type && <div className="text-xs text-slate-500">{e.target.type}</div>}
                      </td>
                      <td className="px-4 py-3 text-slate-300">{e.summary || <span className="text-slate-500">—</span>}</td>
                      <td className="px-4 py-3 text-right text-xs text-slate-500 font-mono">{e.ip || '—'}</td>
                    </tr>
                    {isOpen && (
                      <tr className="bg-slate-900/40">
                        <td colSpan={6} className="px-4 py-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                              <div className="text-xs uppercase tracking-wider text-slate-500 mb-1">Before</div>
                              <pre className="bg-slate-950/70 border border-slate-800 rounded-md p-3 text-xs text-slate-300 overflow-auto max-h-60">{JSON.stringify(e.before || null, null, 2)}</pre>
                            </div>
                            <div>
                              <div className="text-xs uppercase tracking-wider text-slate-500 mb-1">After</div>
                              <pre className="bg-slate-950/70 border border-slate-800 rounded-md p-3 text-xs text-slate-300 overflow-auto max-h-60">{JSON.stringify(e.after || null, null, 2)}</pre>
                            </div>
                          </div>
                          {e.metadata && Object.keys(e.metadata).length > 0 && (
                            <div className="mt-3">
                              <div className="text-xs uppercase tracking-wider text-slate-500 mb-1">Metadata</div>
                              <pre className="bg-slate-950/70 border border-slate-800 rounded-md p-3 text-xs text-slate-300 overflow-auto max-h-40">{JSON.stringify(e.metadata, null, 2)}</pre>
                            </div>
                          )}
