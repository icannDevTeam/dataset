...
function buildInviteUrl(token, preferredBase) {
  // Order of precedence:
  //  1. preferredBase   — usually derived from the live request Host header
  //                       so the link points back to whichever host the admin
  //                       was actually using (prod alias, preview, localhost).
  //  2. PUBLIC_BASE_URL / NEXT_PUBLIC_BASE_URL — explicit env override.
  //  3. VERCEL_URL      — last-resort, deployment-specific (rotates per deploy).
  // No silent hardcoded prod fallback — that caused local-signed tokens to be
  // served via a prod URL, producing bad_signature errors when clicked.
  const candidates = [preferredBase,
    process.env.PUBLIC_BASE_URL,
    process.env.NEXT_PUBLIC_BASE_URL,
    process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null,
    'https://dataset-sigma.vercel.app',
  ];
  const base = candidates.find((candidate) => candidate && !isPrivateHost(candidate));
  if (!base) {
    throw new Error(
...
// Derive a usable absolute base URL (scheme://host) from a Next.js request.
// Honors x-forwarded-proto / x-forwarded-host (set by Vercel's edge).
function baseUrlFromRequest(req) {
  if (!req || !req.headers) return null;
  const h = req.headers;
  const host = h['x-forwarded-host'] || h.host;
  if (!host) return null;
  const requestBase = `${h['x-forwarded-proto'] || (String(host).startsWith('localhost') || String(host).startsWith('127.') ? 'http' : 'https')}://${host}`;
  if (isPrivateHost(requestBase)) return null;
  return requestBase;
}
...
function serialize(doc) {
  ...
  return {
    ...
    token: d.token || null,
    url: storedUrl && !isPrivateHost(storedUrl)
      ? storedUrl
      : (d.token ? buildInviteUrl(d.token) : null),
  };
}
...
async function createInvite(db, tid, opts) {
  ...
  const url = buildInviteUrl(token, opts?.preferredBaseUrl);
...
async function updateInvite(db, tid, lid, patch) {
  ...
  if (allowed.windowOpenAt !== undefined || allowed.windowCloseAt !== undefined) {
    curSnap = await db.doc(inviteDocPath(tid, lid)).get();
    curD = curSnap.exists ? curSnap.data() : {};
  ...
  // Admins can extend the link's lifetime. Two equivalent inputs are
  // accepted: `extendDays` (relative, "+30 days from now") or
  // `expiresAt` (absolute ISO/millis). Re-signs the JWT so the token's
  // own `exp` claim stays in sync with the Firestore record. This
  // means the URL changes — callers (admin UI) must surface that.
  const wantsExtend =
    patch && (
      Object.prototype.hasOwnProperty.call(patch, 'extendDays') ||
      Object.prototype.hasOwnProperty.call(patch, 'expiresAt') ||
      Object.prototype.hasOwnProperty.call(patch, 'ttlDaysFromNow')
    );
  if (wantsExtend) {
    let newExpiresMs = null;
    if (Object.prototype.hasOwnProperty.call(patch, 'expiresAt') && patch.expiresAt) {
      const ts = toTimestamp(patch.expiresAt);
      if (ts) newExpiresMs = ts.toMillis();
    } else {
      const days = Number(patch.extendDays ?? patch.ttlDaysFromNow);
      if (Number.isFinite(days) && days > 0) {
        const clamped = Math.min(Math.max(1, Math.floor(days)), MAX_TTL_DAYS);
        newExpiresMs = Date.now() + clamped * 86400 * 1000;
      }
    }
    if (newExpiresMs && newExpiresMs > Date.now() + 60_000) {
      // Also stretch to cover an updated/existing windowCloseAt + 24h,
      // mirroring createInvite's behaviour so the friendly "window
      // closed" message can render after the window ends.
      if (!curSnap) {
        curSnap = await db.doc(inviteDocPath(tid, lid)).get();
        curD = curSnap.exists ? curSnap.data() : {};
      }
      const effClose =
        (allowed.windowCloseAt !== undefined ? allowed.windowCloseAt : curD?.windowCloseAt) || null;
      if (effClose && typeof effClose.toMillis === 'function') {
        newExpiresMs = Math.max(newExpiresMs, effClose.toMillis() + 86400_000);
      }
      const ttlSeconds = Math.floor((newExpiresMs - Date.now()) / 1000);
      const token = signPickupOnboardingToken({ tenantId: tid, linkId: lid, ttlSeconds });
      const url = buildInviteUrl(token, patch.preferredBaseUrl);
      allowed.expiresAt = admin.firestore.Timestamp.fromMillis(newExpiresMs);
      allowed.token = token;
      allowed.url = url;
      // Re-enable if previously expired so the new lifetime takes effect.
      if (curD && curD.revokedAt == null && allowed.enabled === undefined) {
