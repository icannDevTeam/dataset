...
const { shapePickupEvent, prewarmSession, eventMatchesScopes, SILENT_ON_IPAD } = require('./shape-pickup-event');
...
function _broadcast(tenantId, event) {
  ...
  // Mirror feed.js: audit-only decisions (outside_window, unknown_chaperone,
  // wrong_terminal) must never reach the iPad. Without this, SSE flashes the
  // card for ~2s until the next poll reconciles and drops it.
  if (event && (SILENT_ON_IPAD.has(event.decision) || event.status === 'hidden')) return 0;
  const payload = `event: pickup_event\ndata: ${JSON.stringify(event)}\n\n`;
  let delivered = 0;
  for (const sub of bucket) {
    // Scoped subscribers (iPad / gate UI) should only receive events that
    // explicitly name a terminal in their release group. If terminalId is
    // missing, skip to avoid a brief "flash then disappear" on client reconcile.
    if (sub.terminalIds.size > 0) {
      const terminalMatch = !!(event.terminalId && sub.terminalIds.has(event.terminalId));
      const releaseGroupMatch = !!(sub.releaseGroupId && event.releaseGroupId && sub.releaseGroupId === event.releaseGroupId);
      if (!terminalMatch && !releaseGroupMatch) continue;
    }
    // Grade-scope guard: hide wrong-gate cards (e.g. EY parent scanning at a
    // Grade 4/5 pole) even when the edge writer didn't stamp wrong_terminal.
    if (!eventMatchesScopes(event, sub.gradeScopes)) continue;
 ...
 * @param {string[]} [gradeScopes] — optional grade tokens (e.g. ['4','5']).
 *   When set, events whose students all belong to other grades are dropped.
 * @returns {() => void}
 */
function subscribe(tenantId, terminalIds, res, gradeScopes, releaseGroupId) {
  const sub = {
    res,
    terminalIds: new Set(terminalIds || []),
    gradeScopes: new Set((gradeScopes || []).map((g) => String(g).trim().toUpperCase()).filter(Boolean)),
    releaseGroupId: releaseGroupId ? String(releaseGroupId) : null,
    heartbeat: null,
  };
