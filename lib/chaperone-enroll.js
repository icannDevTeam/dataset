...
export async function enrollChaperone(db, bucket, tid, chaperoneId, opts = {}) {
  ...
  try {
    ...
    // Hikvision terminals reject face pictures >200KB with a misleading
    // badJsonFormat "faceURL" error. Fail fast with an actionable message.
    if (buf.length > 195 * 1024) {
      throw new Error(
        `face photo too large for terminal: ${Math.round(buf.length / 1024)}KB ` +
        '(Hikvision limit 200KB) — replace with a compressed photo and retry'
      );
    }
