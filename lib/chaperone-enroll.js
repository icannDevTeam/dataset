  ...
  try {
    ...
    snap.forEach((d) => {
      ...
      out.push({
        ...
        ip: t.ip,
        gradeScopes: parseGradeLabel(t.gradeLabel),
 ...
 */
function filterDevicesForGrades(devices, chaperoneGrades) {
  const grades = (chaperoneGrades || []).map((g) => String(g).trim()).filter(Boolean);
  if (grades.length === 0) return devices;
  return devices.filter((d) => {
    if (!d.gradeScopes || d.gradeScopes.length === 0) return true; // shared gate
    return d.gradeScopes.some((g) => grades.includes(String(g)));
  });
}
...
export async function enrollChaperone(db, bucket, tid, chaperoneId, opts = {}) {
  ...
  const grades = Array.isArray(opts.gradeFilter) && opts.gradeFilter.length > 0
    ? opts.gradeFilter
    : (Array.isArray(chap.studentGrades) ? chap.studentGrades : []);
  let devices = filterDevicesForGrades(allDevices, grades);
