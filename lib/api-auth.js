...
function withApi(handler, opts = {}) {
  ...
  async function runHandlerSafely(req, res) {
    try {
      return await handler(req, res);
    } catch (err) {
      const msg = err?.message || 'Unhandled API error';
      console.error('[api-auth] unhandled route error:', msg, err?.stack || '');
      _logSecurityEvent(req, 'handler_crash', { err: msg });
  ...
  return async (req, res) => {
    ...
    if (!requireUserFlag) {
      return runHandlerSafely(req, res);
    }
