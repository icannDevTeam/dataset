 ...
 * @param {number} [opts.minDim=600]            upscale threshold in px
 * @returns {Promise<string>} JPEG data-URL guaranteed ≤ maxBytes (best effort)
 */
export async function compressImageToJpegDataUrl(source, { maxDim = 1024, maxBytes = 190 * 1024, minDim = 600 } = {}) {
  ...
  // Tiny sources (e.g. WhatsApp thumbnails) fail Hikvision face modeling with
  // PicFeaturePoints / SubpicAnalysisModelingError — the terminal needs a
  // reasonably large face. Upscale so the longest side is at least ~900px
  // (root-caused 2026-07-30: a 262×296 photo failed on all terminals; the
  // same photo upscaled 3× enrolled everywhere).
  const longest = Math.max(img.width, img.height);
  if (longest < minDim) {
    scale = Math.min(900, maxDim) / longest;
  }
  let best = null;
