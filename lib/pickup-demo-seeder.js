...
function terminalIdFor(deviceName) {
  return crypto.createHash('sha1').update(String(deviceName || '')).digest('hex').slice(0, 12);
}
...
async function runPickupDemoSeed({
  ...
  actorEmail = null,
  eventCount = null,
  classScopes: classScopesOverride = null,
}) {
  ...
  let classScopes = parseClassScopes(Array.isArray(classScopesOverride) ? classScopesOverride.join(',') : classScopesOverride);
  if (classScopes.length === 0) {
    classScopes = await resolveTeacherClassScopes(db, teacherEmail);
  }
  if (classScopes.length === 0) {
