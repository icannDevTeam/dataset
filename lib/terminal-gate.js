...
/**
 * Like `terminalGateStatus` but applies the release group's one-off
 * `pickupOverrides` first when the WIB date matches. Falls back to the
 * terminal's daily window when no override is in effect for today.
 *
 * @param {object} terminal — terminal doc data
 * @param {object|null} group — release group doc data (or null)
 * @param {Date} [now]
 */
function effectiveGateStatus(terminal, group, now = new Date()) {
...
module.exports = { terminalGateStatus, effectiveGateStatus, isValidHHMM, wibDateKey };
