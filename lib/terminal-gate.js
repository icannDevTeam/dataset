...
function terminalWeeklyWindowFor(terminal, now = new Date()) {
  const byDay = terminal?.weeklyWindowByDay;
  if (!byDay || typeof byDay !== 'object') return null;
  const row = byDay[wibWeekdayKey(now)];
  if (!row || typeof row !== 'object') return null;
  if (row.closedAllDay === true) return { closedAllDay: true };
  const open = row.start || row.open;
  const close = row.end || row.close;
 ...
 * Shape:
 *   settings.pickupWindowByDay = {
 *     fri: { start:'10:40', end:'12:30' },
 *     sat: { closedAllDay:true }
 *   }
 */
function weeklyWindowFor(settings, now = new Date()) {
  ...
  if (!w) return null;
  if (w.closedAllDay === true) return { closedAllDay: true };
...
function clampWindowToTerminal(windowLike, terminal) {
  if (!windowLike || windowLike.closedAllDay) return windowLike;
  const termOpen = _hhmmToMinutes(terminal?.windowOpen);
  const termClose = _hhmmToMinutes(terminal?.windowClose);
  const winOpen = _hhmmToMinutes(windowLike.open);
  const winClose = _hhmmToMinutes(windowLike.close);
  if (termOpen == null || termClose == null || winOpen == null || winClose == null) {
    return windowLike;
  }
  if (termOpen > termClose || winOpen > winClose) {
    return windowLike;
  }
  const open = Math.max(termOpen, winOpen);
  const close = Math.min(termClose, winClose);
  if (open > close) {
    return { closedAllDay: true };
  }
  return {
    ...windowLike,
    open: _minutesToHHMM(open),
    close: _minutesToHHMM(close),
  };
}
 ...
 */
function scheduledGateStatus(terminal, group, now = new Date(), settings = null) {
  ...
  if (overrides.length) {
    ...
    if (ov) {
      ...
      const clamped = clampWindowToTerminal({
        open: ov.open || terminal?.windowOpen,
        close: ov.close || terminal?.windowClose,
      }, terminal);
      if (clamped?.closedAllDay) {
        return {
          open: false, manualOverride: null,
          scheduled: { configured: true, opensAt: '', closesAt: '', open: false },
          reason: 'override-out-of-window',
          override: { date: ov.date, open: ov.open, close: ov.close, note: ov.note || null },
        };
      }
      // Build a synthetic terminal that uses the override window, capped by
      // the terminal's own schedule so app-side status never outlives the gate.
      const synthetic = {
        ...terminal,
        windowOpen: clamped?.open || ov.open || terminal?.windowOpen,
        windowClose: clamped?.close || ov.close || terminal?.windowClose,
      };
  ...
  const terminalWeekly = terminalWeeklyWindowFor(terminal, now);
  if (terminalWeekly) {
    if (terminalWeekly.closedAllDay) {
      return {
        open: false,
        manualOverride: null,
        scheduled: { configured: true, opensAt: '', closesAt: '', open: false },
        reason: 'terminal-weekly-closed-all-day',
        override: { weekday: wibWeekdayKey(now), closedAllDay: true, recurring: true, source: 'terminal' },
      };
    }
    const synthetic = {
      ...terminal,
      windowOpen: terminalWeekly.open,
      windowClose: terminalWeekly.close,
    };
    const base = terminalGateStatus(synthetic, now);
    return {
      ...base,
      reason: base.reason === 'in-window' ? 'terminal-weekly-in-window' : (base.reason === 'out-of-window' ? 'terminal-weekly-out-of-window' : base.reason),
      override: { weekday: wibWeekdayKey(now), open: terminalWeekly.open, close: terminalWeekly.close, recurring: true, source: 'terminal' },
    };
  }
  const weekly = weeklyWindowFor(settings, now);
  if (weekly) {
    if (weekly.closedAllDay) {
      return {
        open: false,
        manualOverride: null,
        scheduled: { configured: true, opensAt: '', closesAt: '', open: false },
        reason: 'weekly-closed-all-day',
        override: { weekday: wibWeekdayKey(now), closedAllDay: true, recurring: true },
      };
    }
    const clamped = clampWindowToTerminal(weekly, terminal);
    if (clamped?.closedAllDay) {
      return {
        open: false,
        manualOverride: null,
        scheduled: { configured: true, opensAt: '', closesAt: '', open: false },
        reason: 'weekly-out-of-window',
        override: { weekday: wibWeekdayKey(now), open: weekly.open, close: weekly.close, recurring: true },
      };
    }
    const synthetic = {
      ...terminal,
      windowOpen: clamped?.open || weekly.open,
      windowClose: clamped?.close || weekly.close,
    };
    const base = terminalGateStatus(synthetic, now);
    return {
      ...base,
...
function effectiveGateStatus(terminal, group, now = new Date(), settings = null) {
  const manualOverride = terminal?.gateOverride === 'open' || terminal?.gateOverride === 'closed'
    ? terminal.gateOverride
    : null;
  const scheduleTerminal = manualOverride ? { ...terminal, gateOverride: null } : terminal;
  const status = scheduledGateStatus(scheduleTerminal, group, now, settings);

  if (!manualOverride) return status;
  return {
    ...status,
    open: manualOverride === 'open',
    manualOverride,
    reason: manualOverride === 'open' ? 'manual-open' : 'manual-closed',
  };
}
